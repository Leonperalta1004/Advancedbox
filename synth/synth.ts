...
export class Song {
    ...
    private static readonly _latestEdoBoxVersion: number = 3;
    ...
    public toBase64String(): string {
        ...
        buffer.push(SongTagCode.scale, base64IntToCharCode[Math.floor(this.scale / 64)], base64IntToCharCode[this.scale % 64]);
    ...
    public fromBase64String(compressed: string): void {
        ...
        const beforeThreeEdoBox: boolean = version < 3;
        ...
        while (charIndex < compressed.length) switch (command = compressed.charCodeAt(charIndex++)) {
            ...
            case SongTagCode.key: {
                if (beforeSeven && fromBeepBox) {
                    this.key = clamp(0, this.edo, 11 - base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                } else {
        ...
        if (instrument.type == InstrumentType.fm) {
            ...
            for (let i: number = 0; i < Config.operatorCount; i++) {
                ...
                if (i < carrierCount) {
                    ...
                    const pitchExpressionEnd: number = Math.pow(2.0, -((pitchEnd - expressionReferencePitch) * 12 / edo_) / pitchDamping);
            ...
            const pitchExpressionEnd: number = Math.pow(2.0, -((endPitch - expressionReferencePitch) * 12 / edo_) / pitchDamping);
            ...
            if (instrument.type == InstrumentType.chip || instrument.type == InstrumentType.customChipWave || instrument.type == InstrumentType.harmonics || instrument.type == InstrumentType.pickedString) {
                ...
                const unisonAStart: number = Math.pow(2.0, (unison.offset + unison.spread) * unisonEnvelopeStart / 12);
                const unisonAEnd: number = Math.pow(2.0, (unison.offset + unison.spread) * unisonEnvelopeEnd / 12);
                const unisonBStart: number = Math.pow(2.0, (unison.offset - unison.spread) * unisonEnvelopeStart / 12) * specialIntervalMult;
                const unisonBEnd: number = Math.pow(2.0, (unison.offset - unison.spread) * unisonEnvelopeEnd / 12) * specialIntervalMult;
