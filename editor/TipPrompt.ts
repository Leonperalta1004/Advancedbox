	...
	constructor(private _doc: SongDocument, type: string) {
		...
		switch (type) {
			case "scale": {
				message = div(
					h2("Scale"),
					p("The scale setting limits which notes are available for use. Setting a scale for your music helps tie the notes together. If you're deliberately composing microtonal music, I'm going to assume you know what a scale is. If you don't, then see JummBus's explanation at JummBox (https://jummb.us)"),
					p("Scale names for the heptatonic and octatonic scales are taken from the Xenharmonic Wiki (https://en.xen.wiki), or by taking the names from there and adding the prefix \"anti-\". The nonatonic scale names are my own names, inspired by the ones listed there (I only want to use place names for scale modes). Scale names for pentatonic scales are taken from corresponding modes of the heptatonic scales and suffixed with \"Pentatonic\", or by attaching \"anti-\" to names of opposite scales.")
				);
			} break;
			case "key": {
				message = div(
